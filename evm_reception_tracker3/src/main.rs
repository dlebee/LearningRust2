...
async fn main() -> Result<(), String> {
    ...
    let _transfer_watcher = TransferWatcherService::try_initialize(network_service.clone(), block_watcher.block_rx.resubscribe(), stop_token.clone()).await?;
    let _pending_transaction_watcher = PendingTransactionWatcherService::try_initialize(network_service.clone(), stop_token.clone()).await?;
